...
{
    public async Task<bool> ScheduleExist(Guid scheduleId, Guid userId, CancellationToken cancellationToken)
    {
        return await context.Schedules.AnyAsync(x => x.Id == scheduleId && x.UserId == userId, cancellationToken);
    }

    public async Task<List<GetDaysResponse>> GetDays(Guid scheduleId, CancellationToken cancellationToken)
    {
