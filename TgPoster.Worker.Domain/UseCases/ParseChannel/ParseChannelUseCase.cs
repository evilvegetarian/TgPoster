				...
				foreach (var message in al.Value)
				{
					...
					if (message.media is not null && !deleteMedia)
					{
						if (message.media is MessageMediaPhoto { photo: Photo photo })
						{
							...
							await telegramExecuteServices.DownloadPhotoAsync(client, channel, photo, stream,
								message.ID);
							stream.Position = 0;
							var photoStream = new InputFileStream(stream);
							var photoMessage =
								await telegramExecuteServices.SendPhoto(telegramBot, chatId, photoStream, 3, ct);
						...
						else if (message.media is MessageMediaDocument { document: Document doc })
						{
							...
							if (fileType == "video")
							{
								...
								await telegramExecuteServices.DownloadVideoAsync(client, channel, doc, stream,
									message.ID);
								...
								var messages =
									await telegramExecuteServices.SendMedia(telegramBot, chatId, album, 3, ct);
